	...
	private Map<String, Object> regist(String thirdPartToken,
		...
		try {
			...
			if (ruser.getId() != null && ruser.getId() != 0) {
				// 新注册的用户默认关注 我拍小助手lj
				String officialUserId = jedisService
						.get(BicycleConstants.OFFICICAL_USER_ID);
				if (officialUserId != null && !"".equals(officialUserId)) {
					if (attentionService.isAttention(ruser.getId(),
							Long.parseLong(officialUserId)) != 1) {
						int result = attentionService.payAttention(
								ruser.getId(), Long.parseLong(officialUserId),
								"");
						if (result == 1) {// 添加关注成功，放到redis里
							jedisService.setValueToSetInShard(
									BicycleConstants.ATTENTION_ID_OF
											+ ruser.getId(), officialUserId);
						}
					}
				}
				Long score = System.currentTimeMillis();
				jedisService.setValueToSortedSetAndDel(
						BicycleConstants.NEW_REGISTER_USER, score,
						String.valueOf(ruser.getId()), score - 24 * 60 * 60
								* 1000);
