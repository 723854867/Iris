package com.busap.vcs.util99bill;
...
//			FileInputStream ksfis = new FileInputStream("e:/tester-rsa.pfx");
			...
			// 读取密钥仓库（相对路径）
//			String file = Pkipair.class.getResource("tester-rsa.pfx").getPath().replaceAll("%20", " ");//demo文件原值
...
//			char[] keyPwd = "123456".toCharArray();//demo文件原值
			char[] keyPwd = "busonline10020".toCharArray();
	...
	public boolean enCodeByCer( String val, String msg) {
		...
		try {
			...
			//获得文件(相对路径)
//			String file = Pkipair.class.getResource("99bill[1].cert.rsa.20140803.cer").toURI().getPath();//demo文件原值
