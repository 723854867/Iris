		...
		$.ajax({
			...
			success: function(data){
				if(data!=null&&data.result!=null){
	                ...
	                playVideo('${video.playKey}','${video.nameReplace}','${video.descriptionReplace}','${video.user.name}',
	                ...
	                setfenxiang('${video.playKey}','${video.user.name}','${video.descriptionReplace}','${video.videoPic}')
					}
				}
			});
        			...
        			success: function(data){
        				if(data!=null&&data.result!=null){
        					...
        					for(var i=0;i<listurl.length;i++){
        						...
        						var userPic;
        						var userv="";
            					if(evaluation.user!=null&&evaluation.user.pic!=null){
             					...
             					if(evaluation.user!=null&&(evaluation.user.vipStat==1||evaluation.user.vipStat=='1')){
                                	userv=" <span id='lanv'><img class='lanv1' width='13' height='13' src='<%=imageurl%><%=request.getContextPath()%>/img/lanv.png'/></span>";
                                }else if(evaluation.user!=null&&(evaluation.user.vipStat==2||evaluation.user.vipStat=='2')){
            ...
            function addevaluation(){
              ...
              function addpraise(){
            	...
            	var videoId = $("#videoId").val();
            	var addPraise="../praise/savePraise";
            ...
            function openlogin(){
            	...
            	window.location.href="<%=request.getContextPath()%>/h5/login.jsp?backurl=<%=request.getContextPath()%>/html5/videodetail?videoId="+nowplayid;
            	
            }
            function openregister(){
            	var nowplayid=$("#videoId").val();
            	window.location.href="<%=request.getContextPath()%>/h5/reg.jsp?backurl=<%=request.getContextPath()%>/html5/videodetail?videoId="+nowplayid;
	...
	<div class="f-bottom2">
        ...
        <ul class="tab-bd3">
			...
			<c:if test="${status.count%2==1}">
			 <div class="picimg fl"
            onclick="playVideo('${one.playKey}','${one.nameReplace}','${one.descriptionReplace}','${one.user.name}',
                    		...
                    		'<fmt:formatDate value="${one.createDateStr}" pattern="MM-dd HH:mm"/>','${one.videoPic}','${one.id}','${one.user.vipStat}','${one.praiseCount}','${one.evaluationCount}');setfenxiang('${one.playKey}','${one.user.name}','${one.descriptionReplace}','${one.videoPic}');"
            >
