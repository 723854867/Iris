			...
			if(data.code== 200){
				$.each(data.result,function(m,n){
					...
					if(n.isGive=='0'){
						...
						"<img src='"+_this.picSouce+"/dist/img/gold.png' class='gold-icon'/>"+
						"<span class='list-left'>"+ n.name+"</span>"+
						"<span class='list-middle'>赠"+ n.giveCount+"金币</span>"+
						"<span class='list-right'>￥"+ n.price+"</span>"+
						"</li>");
					}
			...
			if(!_this.id && _this.id == ''){
				...
				$.postDate(_this.idverification,'userId='+_this.id,function(data){
					if(data.code == 200){
						...
						$('.dialog-userpic').attr('src',_this.httpUrl+data.result.pic);
						$('.dialog-username').html(data.result.name);
						$('.dialog-user-id').html(data.result.id)

						$('.dialog-user-info,.skyBox').show();

						$('body').on('click','.dialog-sub',function(){
						//$('.dialog-sub').click(function(){
							$('.user-name,.user-name2').html(data.result.name);
							$('.user-pho').attr('src',_this.httpUrl+data.result.pic);
							$('.one').hide();
							$('.two').show();
							$('.dialog-user-info,.skyBox').hide();
							$.setCookie('userId',_this.id);
						})
