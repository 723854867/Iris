$.extend({
	init: function(){
		...
		//接口数据
		_this.postUrl2= _this.serverUrl+"/school/getCode"; //获取验证码（手机号）  提测试的时候要加上 /resth5/
		_this.postUrl4= _this.serverUrl+"/school/register"; //提交信息    提测试的时候要加上 /resth5/
		var tempData= new Date().getTime();
		var skyH= $(window).height();
		var skyW= $(window).width();
		$('.sub_ok').height($(document).height());
		$('.select-list1').width(skyW-2);
	//	逻辑判断层

		$.wordNumFn($('.item-txtarea'),$('.word_num'),30);
	...
	//	return sexVal;
	//},
	wordNumFn: function(obj,numBox,numVal){
		$(obj).focus(function(){
			var times= null;
			times=setInterval(function(){
				...
				$(numBox).html(cha);
				if(len >= numVal){
					var newStr= $(obj).val().substring(0,numVal);
					$(obj).val(newStr);
				}
			},100)
			$(obj).blur(function(){
