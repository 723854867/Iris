define(['../libs/jquery-2.1.4', '../common/ajax'], function ($, Ajax) {
    return {
        ...
        getRoomUser:function(page,size,roomId){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/live/getRoomUser',{
        		page:page,
        		size:size,
        		roomId:roomId
        	})
        ...
        getRoomById:function(roomId){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/live/getRoomById',{
        		roomId:roomId
        	})
        },
        getLiveInfoByUserId:function(userId){//根据用户id获取直播
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/attention/getLiveInfoByUserId',{
        		userId:userId
        	})
        },
        //获得直播及回放列表
        getLiveAndRecordList:function(page,size,type,liveActivityId){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/live/getLiveAndRecordList',{
        		page:page,
        		size:size,
        		type:type,
        ...
        //根据id获得直播活动信息
        findLiveActivityById:function(id){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/liveActivity/findLiveActivityById',{
        		id:id
        	})
        },
        //直播互动排行榜
        getRank:function(id,page,size){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/liveActivity/getRank',{
        		id:id,
        		page:page,
        		size:size
        	})
        },
        isLive:function(roomId){
        	return Ajax.quiet().postJSONWithBeforeSend('/restwww/live/isLive',{
        		roomId:roomId
        	})
        }
    };
