            ...
            $('#ttLiving').datagrid({
                ...
                columns: [
                    [
                        ...
                        {
                            field: 'roomPic',
                            title: '<span class="columnTitle">房间图片</span>',
                            width: 100,
                            align: 'center',
                            formatter: function (value, row) {
                                return '<img src="/restadmin/download' + value + '" style="width:100px;height:100px;">';
                        ...
                        {
                            field: 'anchorName',
                            title: '<span class="columnTitle">主播昵称</span>',
                            width: 120,
                            align: 'center'
                        },
                        {
                            field: 'creatorId',
                            title: '<span class="columnTitle">主播ID</span>',
                            width: 80,
                            align: 'center'
                        },
                        {
                            field: 'praiseNumber',
                            title: '<span class="columnTitle">点赞数</span>',
                            width: 80,
                            align: 'center'
                        },
                        {
                            field: 'onlineNumber',
                            title: '<span class="columnTitle">当前在线人数</span>',
                            width: 80,
                            align: 'center'
                        },
                        {
                        ...
                        {
                            field: 'createDate',
                            title: '<span class="columnTitle">开始时间</span>',
                            width: 80,
                            align: 'center'
                        }
                    ]
...
<div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'center',border:false" style="padding:5px;margin:0 auto;">
        <form id="myform" method="post" enctype="multipart/form-data" action="banner/modify">
            <input type="hidden" name="id" value="${banner.id}">
            <table cellpadding="5" style="margin:0 auto;width:900px;text-align: left;" class="table-doc">
                <tr>
                    <td style="width: 20%;">标题：</td>
                    <td style="width: 80%;text-align: left;">
                        <input class="easyui-validatebox easyui-textbox" name="title" id="title"
                               data-options="validType:'length[0,20]'" required="true" value="${banner.title}">
                    </td>
                </tr>
                <tr>
                    <td>Banner图:</td>
                    <td style="text-align: left;">
                        <div style="float: left;height: 100px;width: 220px;">
                            <input name="file" id="imgSrc" type="file" required="true" value="">
                        </div>
                        <div style="float: left;margin-left:20px;height: 100px;width: 100px;">
                            <img alt="" src="/restadmin/download${banner.imgSrc}" style="width: 100px;height: 100px;">
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>banner类型</td>
                    <td style="text-align: left;">
                        <input type="radio" value="1" name="bannerType" <c:if test="${banner.bannerType eq 1}">checked="checked"</c:if>>首页
                        <input type="radio" value="2" name="bannerType" <c:if test="${banner.bannerType eq 2}">checked="checked"</c:if>>发现页
                        <input type="radio" value="3" name="bannerType" <c:if test="${banner.bannerType eq 3}">checked="checked"</c:if>>首页新歌声
                        <input type="radio" value="4" name="bannerType" <c:if test="${banner.bannerType eq 4}">checked="checked"</c:if>>新歌声专区
                        <input type="radio" value="5" name="bannerType" <c:if test="${banner.bannerType eq 5}">checked="checked"</c:if>>学员榜
                        <input type="radio" value="6" name="bannerType" <c:if test="${banner.bannerType eq 6}">checked="checked"</c:if>>综艺榜
                        <input type="radio" value="7" name="bannerType" <c:if test="${banner.bannerType eq 7}">checked="checked"</c:if>>综艺预告榜
                        <input type="radio" value="8" name="bannerType" <c:if test="${banner.bannerType eq 8}">checked="checked"</c:if>>主播榜
                        <input type="radio" value="9" name="bannerType" <c:if test="${banner.bannerType eq 9}">checked="checked"</c:if>>贡献榜
                        <input type="radio" value="10" name="bannerType" <c:if test="${banner.bannerType eq 10}">checked="checked"</c:if>>贡献总榜
                        <input type="radio" value="11" name="bannerType" <c:if test="${banner.bannerType eq 11}">checked="checked"</c:if>>本期人气
                        <input type="radio" value="0" name="bannerType" <c:if test="${banner.bannerType eq 0 || banner.bannerType eq ''}">checked="checked"</c:if>>其它
                    </td>
                </tr>
                <tr>
                    <td>点击操作:</td>
                    <td style="text-align: left;">
                        <select name="targetType" id="targetType" style="width:50%" onchange="chgOpration()">
                            <option value="activity"
                                    <c:if test="${banner.targetType eq 'activity'}">selected="selected"</c:if>>活动详情
                            </option>
                            <option value="video"
                                    <c:if test="${banner.targetType eq 'video'}">selected="selected"</c:if>>视频详情
                            </option>
                            <option value="user" <c:if test="${banner.targetType eq 'user'}">selected="selected"</c:if>>
                                个人中心
                            </option>
                            <option value="h5" <c:if test="${banner.targetType eq 'h5'}">selected="selected"</c:if>>
                                H5页面
                            </option>
                            <option value="live" <c:if test="${banner.targetType eq 'live'}">selected="selected"</c:if>>
                                直播推荐
                            </option>
                            <option value="revideo"
                                    <c:if test="${banner.targetType eq 'revideo'}">selected="selected"</c:if>>回放
                            </option>
                            <option value="liveAct"
                                    <c:if test="${banner.targetType eq 'liveAct'}">selected="selected"</c:if>>直播活动
                            </option>
                            <option value="hotLabel"
                                    <c:if test="${banner.targetType eq 'hotLabel'}">selected="selected"</c:if>>热门话题
                            </option>
                        </select>
                    </td>
                </tr>
                <tr id="h5_tr" <c:if test="${banner.targetType ne 'h5' && banner.targetType ne 'hotLabel'}">style="display:none"</c:if>>
                    <c:choose>
                        <c:when test="${banner.targetType eq 'h5'}">
                        <td id="h5_td">H5页面地址:</td>
                        </c:when>
                        <c:when test="${banner.targetType eq 'hotLabel'}">
                        <td id="h5_td">热门话题名称:</td>
                        </c:when>
                    </c:choose>

                    <td style="text-align: left;">
                        <input name="targetUrl" id="targetUrl" onfocus="showTargetUrlDialog()" value="${banner.targetUrl}">
                    </td>
                </tr>

                <tr id="common_tr" <c:if test="${banner.targetType eq 'h5' || banner.targetType eq 'hotLabel'}">style="display:none"</c:if>>
                    <td>跳转位置:</td>
                    <td style="text-align: left;">
                        <input name="targetId" id="targetid" value="${banner.targetId}" onfocus="showTargetDialog();">
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td style="text-align: left;">
                        <select name="tag" id="tag" style="width:50%" class="easyui-combobox">
                            ...
                            <option value="0" <c:if test="${banner.tag==0}">selected="selected"</c:if>>hot</option>
                            <option value="1" <c:if test="${banner.tag==1}">selected="selected"</c:if>>new</option>
                            <option value="2" <c:if test="${banner.tag==2}">selected="selected"</c:if>>火爆</option>
                            <option value="3" <c:if test="${banner.tag==3}">selected="selected"</c:if>>预告</option>
                            <option value="4" <c:if test="${banner.tag==4}">selected="selected"</c:if>>直播</option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td>显示顺序:</td>
                    <td style="text-align: left;">
                        <input name="orderNum" id="orderNum" value="${banner.orderNum}" required="true">
                    </td>
                </tr>

                <tr>
                    <td colspan="2" align="center">
                        <input type="submit" style="width: 50px;height:25px" class="button green bigrounded"
                               data-options="iconCls:'icon-ok'" onclick="clickupload()" value="保存"/>
                        <a href="banner/listbanner" style="" class="easyui-linkbutton"
...
<div id="livingDlg" class="easyui-dialog" style="width:800px;height:650px;padding:10px 20px" closed="true"
     buttons="#ldlg-buttons">
...
<div id="ldlg-buttons">
    ...
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel"
       onclick="javascript:$('#livingDlg').dialog('close')">取消</a>
</div>
