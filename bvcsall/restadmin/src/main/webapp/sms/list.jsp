...
<script type="text/javascript">
     ...
     var listUrl="sms/listpage";
     var updateUrl="sms/updatepage";
     var deleteUrl="sms/delete";
     var addUrl="sms/create";
	...
	$(function() {
	    ...
	    url:listUrl,
	    columns:[[
				{field:'name',title:'被通知人名称',width:100},
				{field:'phoneNo',title:'手机号',width:100},
		    ]],
		});
	});
	...
	function saveUser(mydialogueId,myFormId){
		...
		if(mydialogueId == "#dlg"){//新增
			var save_tel = $('#save_tel').numberbox('getValue');
			if (!patrn.exec(save_tel)){
				showMessage("提示","请输入合法的手机号");
				return false
			} 
		}else{
			var edit_tel = $('#edit_tel').numberbox('getValue');
			if (!patrn.exec(edit_tel)){
				showMessage("提示","请输入合法的手机号");
				return false
			} 
		}
...
<body>
	...
	<div id="dlg" class="easyui-dialog" style="width:400px;height:380px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		...
		<form id="fm" method="post" novalidate>
			...
			<div class="fitem">
				<label>手机号:</label>
				<input id="save_tel" name="phoneNo" class="easyui-numberbox" required="true" maxlength="11" onkeypress="jump(event,'#dlg','#fm');">
			</div>
		</form>
	</div>
	...
	<div id="updatedlg" class="easyui-dialog" style="width:400px;height:380px;padding:10px 20px" closed="true" buttons="#updatedlg-buttons">
		<div class="ftitle">修改被通知人</div>
		<!-- 修改 -->
		<form id="updatefm" method="post" novalidate>
			<div class="fitem">
				<label>名称:</label>
				<input name="name" class="easyui-validatebox" required="true" onkeypress="jump(event,'#updatedlg','#updatefm');">
			</div>
			<div class="fitem">
				<label>手机号:</label>
				<input id="edit_tel" name="phoneNo" class="easyui-numberbox" required="true" maxlength="11" onkeypress="jump(event,'#dlg','#fm');">
			</div>
