...
<head>
    ...
    <script type="text/javascript">
        ...
        var listUrl = "recommandPosition/getresult";
        var updateUrl = "recommandPosition/updateTemp";
        var deleteUrl = "recommandPosition/deleteTemp";
        var addUrl = "recommandPosition/create";
        ...
        $(function () {
            ...
            $(datagridId).datagrid({
                ...
                columns: [[{
                    ...
                    formatter: function (value, row, index) {
                        if ($.trim(row.picPath) != "") {
                            return "<img src='/restadmin/download" + row.picPath + "' width='80' height='120'/>";
                        }
                    }
                }, {
                    field: 'title',
                    ...
                    formatter: function (value, row, index) {
                        if (row.status == 0) {
                            return "<a style='color:red;' href='javascript:;' onclick='settingStatus("+row.id+",1)'>启用</a>";
                        } else if (row.status == "1") {
                            return "<a href='javascript:;' onclick='settingStatus("+row.id+",0)'>停用</a>";
                        } else {
                            return "已过期";
                        }
                    }
        ...
        function destroyUser(id) {
            $.messager.confirm('删除推荐位', deleteConfirmMessage, function (r) {
                if (r) {
                    $.get(deleteUrl, {
                        id: id
                    }, function (result) {
        ...
        function editUser(index) {
            ...
            if (row) {
                ...
                $('#image_file').attr("src", "/restadmin/download" + row.picPath);
        ...
        function saveUser(mydialogueId, myFormId, selectId, optType) {
            if (optType == "add") {
                ...
                if (len1 == 0) {
                    showMessage("Error", "请选择推荐位封面");
                    return false;
                } else {
                    var temp = eval("file1");
                    var fileext = temp.substring(temp.lastIndexOf("."));
                    fileext = fileext.toLowerCase();
                    if (fileext != '.jpg' && fileext != '.gif' && fileext != '.jpeg' && fileext != '.png' && fileext != '.bmp') {
                        showMessage("Error", "对不起，封面图仅支持标准格式的图片，请不要调皮!O(∩_∩)O谢谢~");
                        return false;
                    }
                }
        ...
        function Invalid() {
            ...
            if (confirm("确定要让其失效吗？")) {
                $.ajax({
                    url: 'recommandPosition/invalidAll',
                    data: {
                        id: ids
                    },
                    type: "post",
        ...
        function selectLabel() {
            var aid = $("#labels option:selected").val();
        ...
        function settingStatus(id, status) {
            $.ajax({
                url: 'recommandPosition/settingStatus',
...
<div id="tb" region="north" border="false"
    ...
    <div data-options="region:'north',border:false"
        ...
        <h2 style="float:left;padding-left:10px;margin: 1px">推荐位管理</h2>
    </div>
...
<div id="dlg" class="easyui-dialog" style="width:700px;height:520px;padding:2px 5px;" closed="true"
     buttons="#dlg-buttons">
    <form id="fmadd" method="post" action="recommandPosition/add" novalidate enctype="multipart/form-data">
        <table class="table-doc">
            <tr>
                ...
                <td width="35%">
                    <select name="operation" style="width: 150px;" class="easyui-combobox" id="operation_add" onchange="chgOpration(this)">
                        <option value="label">热门话题视频列表</option>
            ...
            <tr>
                <td><label>推荐页号:</label></td>
                <td><input id="page" name="page" required="true" style="width: 150px;"></td>
                <td><label>推荐位置:</label></td>
                <td><input id="position" name="position" required="true" style="width: 150px;"></td>
            </tr>
            <tr>
...
<!-- 弹出的选择活动对话框 -->
<div id="act_dlg" class="easyui-dialog" style="width:250px;height:250px;padding:5px 10px" closed="true"
...
<!-- 弹出的话题对话框 -->
<div id="label_dlg" class="easyui-dialog" style="width:250px;height:250px;padding:5px 10px" closed="true"
     buttons="#lbdlg-buttons">
    话题&nbsp;&nbsp;
    <select id="labels">
...
<div id="updatedlg" class="easyui-dialog" style="width:700px;height:600px;padding:2px 5px" closed="true"
    ...
    <form id="updatefm" method="post" action="recommandPosition/updateTemp" novalidate enctype="multipart/form-data">
        <table class="table-doc" width="100%">
            <tr>
                ...
                <td>
                    <select name="operation" id="operation_edit" style="width: 150px;" class="easyui-combobox"
            ...
            <tr>
                <td>
                    <img src="" alt="封面图" id="image_file" style="width: 60px;height: 60px;">
                </td>
                <td colspan="3">选择封面
                    <input class="file" type="file" id="file2" name="file"/>
                </td>
            </tr>
        </table>
    </form>
