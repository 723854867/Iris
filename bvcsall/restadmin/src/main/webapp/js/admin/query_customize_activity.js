 ...
 * Created by zx
 */
var listUrl = "customizeActivity/queryCustomizeActivityList";
    ...
    $('#displayTable').datagrid({
        ...
        columns: [
            [
                ...
                {
                    field: 'actPicIos', title: '<span class="columnTitle">活动图片</span>', width: 150,align:'center',
                ...
                {
                    field: 'details',
                    title: '<span class="columnTitle">有效时间</span>',
                    width: 150,
                    align: 'center',
                    formatter: function (value, row) {
                    	
                    	
                        var createDateParam = "<span>" + formatterdate(row.timeStart, row) +"    --    "+formatterdate(row.timeEnd, row) + "</span>";
                ...
                },
                {
                    ...
                    formatter: function (value, row) {
                    	var optionStr ="";
                        var editStr = '<a class="easyui-linkbutton" href="customizeActivity/showCustomizeActivityAdd?caId=' + row.id + '" style="width:80px;height: 25px;" id="mdOM' + row.id + '" name="mdOM' + row.id + '" type="button" title="编辑" ><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">编辑</span><span class="l-btn-icon icon-edit">&nbsp;</span></span></a>';
                        if (row.status == 2) {
                            optionStr = '<div><a class="easyui-linkbutton" plain="true" href="javascript:;" style="width:80px;height: 30px;" id="mdOM' + row.id + '" name="mdOM' + row.id + '" value="上线" onclick="updateActiveStatus(this,\'' + row.id + '\',1);"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">上线</span><span class="l-btn-icon icon-add">&nbsp;</span></span></a>' + " "
                                + editStr ;
                        } else {
                            optionStr = '<div><a class="easyui-linkbutton" plain="true" href="javascript:;" style="width:80px;height: 30px;" id="mdOM' + row.id + '" name="mdOM' + row.id + '" value="下线" onclick="updateActiveStatus(this,\'' + row.id + '\',2);" ><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">下线</span><span class="l-btn-icon icon-remove">&nbsp;</span></span></a>' + " "
                                + editStr;
                        }
                        return optionStr;
                    }
...
function updateActiveStatus(obj, activityId, status) {
    ...
    $.ajax({
        url: 'customizeActivity/updateActiveStatus',
