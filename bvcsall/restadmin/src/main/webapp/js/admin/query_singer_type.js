...
function doSearch() {
    ...
    $('#displayTable').datagrid({url: "song/querySingerTypeList"});
}
...
function dialogUpdate(value) {
    var title = '请求明细';
    var url = 'song/updateSingerTypeTemplate?id=' + value;
    initWindow(title, url, 500, 170);
}

function doUpdate() {
    $('#updateForm').form('submit', {
        url: "song/updateSingerType",
        success: function (response) {
            var parsedJson = jQuery.parseJSON(response);
            if (parsedJson.resultCode == "success") {
                showMessage("提示信息", parsedJson.resultMessage);
