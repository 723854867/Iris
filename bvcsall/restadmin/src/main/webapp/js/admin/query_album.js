...
function doSearch() {
    ...
    $('#displayTable').datagrid({url: "song/queryAlbumList"});
}
...
function dialogUpdate(value) {
    var title = '请求明细';
    var url = 'song/updateAlbumTemplate?id=' + value;
    initWindow(title, url, 500, 210);
}

function doUpdate() {
    $('#updateForm').form('submit', {
        url: "song/updateAlbum",
        success: function (response) {
            var parsedJson = jQuery.parseJSON(response);
            if (parsedJson.resultCode == "success") {
                showMessage("提示信息", parsedJson.resultMessage);
