...
var listUrl = "liveComplaint/searchpagelist";
...
$(function () {
    $(datagridId).datagrid({
        ...
        columns: [[
            ...
            {
                field: 'liveImg', title: "缩略图", width: 100, align: 'center', formatter: function (value, row, index) {
                var videoPlayUrlPrefix = $("#uploadpic_url_prefix").val();
                return "<img src='/restadmin/download"  + value + "' width='120' height='100' />";
            }
            },
            {field: 'liveTitle', title: "直播名称", width: 100, align: 'center'},
            {field: 'complaints', title: "投诉人", width: 100, align: 'center'},
            {field: 'createDate', title: "投诉时间", width: 60, align: 'center',
            ...
            {field: 'content', title: "投诉类型", width: 40, align: 'center'},
            {
                field: 'stat', title: '处理状态', width: 40, align: 'center', formatter: function (value, row, index) {
                var stat = "未处理";
                if (row.stat == 1) {
                    stat = "已处理";
                }
            ...
            {field: 'liveUserName', title: "直播人", width: 100, align: 'center'},
            {
                field: 'id', title: "操作", width: 100, align: 'center', formatter: function (value, row, index) {
                var content = "";
                if (row.stat != 1) {
                    content += '<a class="easyui-linkbutton" onClick="dealComplaints(' + row.id + ',1)" href="javascript:;" style="width:80px;height: 25px;" title="忽略"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">忽略</span><span class="l-btn-icon icon-cancel">&nbsp;</span></span></a>';
                
                    content += '<a class="easyui-linkbutton" onClick="dealComplaints(' + row.id + ',2)" href="javascript:;" style="width:80px;height: 25px;" title="下线"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">下线</span><span class="l-btn-icon icon-remove">&nbsp;</span></span></a>';
                    
                    content += '<a class="easyui-linkbutton" onClick="dealComplaints(' + row.id + ',3)" href="javascript:;" style="width:80px;height: 25px;" title="封号"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">封号</span><span class="l-btn-icon icon-remove">&nbsp;</span></span></a>';
                }
