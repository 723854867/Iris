...
public class AnchorWeekRankingScheduler {

	/*
	 * @Autowired private SchedulerFactoryBean schedulerFactoryBean;
	 */
	private QuartzProducer quartzProducer;
	private static String JOB_GROUP_NAME = "anchor_week_ranking_group";
	private static String JOB_NAME = "anchor_week_ranking_job";
	...
	public void initDelayJob() {
		try {
			...
			JobDetail jobDetail = new JobDetail(JOB_NAME, JOB_GROUP_NAME, AnchorWeekRankingTask.class);// 任务名，任务组，任务执行类
			Trigger trigger = new CronTrigger(JOB_NAME, JOB_GROUP_NAME, "0 0/10 * * * ?");
