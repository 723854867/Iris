...
public class ReCalculateUserPopularityRank {

	/*
	 * @Autowired private SchedulerFactoryBean schedulerFactoryBean;
	 */
	private QuartzProducer quartzProducer;
	private static String JOB_GROUP_NAME = "calculate_popularity_group";
	private static String JOB_NAME = "calculate_popularity_job";
	...
	public void initDelayJob() {
		try {
			...
			JobDetail jobDetail = new JobDetail(JOB_NAME, JOB_GROUP_NAME, ReCalculatPopularityTask.class);// 任务名，任务组，任务执行类
			Trigger trigger = new CronTrigger(JOB_NAME, JOB_GROUP_NAME, "0 0 1 * * ?");//每天凌晨1点执行
