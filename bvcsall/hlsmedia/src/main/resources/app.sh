...
##################### 一般情况下需要设置#####################
# JAVA程序的名称，不能重复 
PROG=bvcs-hls-server
# JVM 环境
JAVA_HOME=/home/maishi/jdk1.7.0
JAVA_HOME=${JAVA_HOME-/home/mycall/soft/jdk1.7.0}
jvmsettings="-server -Xms8G -Xmx8G -Xss256k -XX:PermSize=64M -XX:MaxPermSize=64M -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:InitiatingHeapOccupancyPercent=80 -XX:MaxTenuringThreshold=20 -XX:+OptimizeStringConcat -XX:-RelaxAccessControlCheck  -XX:+UseCompressedOops  -XX:+HeapDumpOnOutOfMemoryError  -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:gc.log -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=9086 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false"
# echo "${JAVA_HOME}/bin/java ${jvmsettings} ${parameters}"
# 运行参数
parameters="-jar server.jar -Dctxconfig server.properties -Dlogback logback.xml"

##################### 额外处理####################
# 判断ffmpeg是否解压缩
if [ -f "./dependency/linux64_x86/ffmpeg.tgz" ]; then 
  if [ ! -x "./dependency/linux64_x86/ffmpeg/bin/ffmpeg" ]; then
      tar -zxf ./dependency/linux64_x86/ffmpeg.tgz -C ./dependency/linux64_x86/
  fi
fi  
mkdir -p ./files
##################### 一般情况下不需要设置####################
