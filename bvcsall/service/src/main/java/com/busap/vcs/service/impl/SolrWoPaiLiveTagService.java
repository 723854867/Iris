...
@Service
public class SolrWoPaiLiveTagService {
	...
	public SolrWoPaiLiveTagService(){
	...
	public void init(){
		...
		solr.setDefaultCollection("wopailivetag");
	...
	public void index(String id,String name,long videoId,long num) throws SolrServerException, IOException{  
		//创建doc文档
	    SolrInputDocument doc = new SolrInputDocument();
	    doc.addField("id", id);
	    doc.addField("name", name);
	    doc.addField("liveid", videoId);
	...
	public List<Long> search(String name,Integer start,Integer rows) throws SolrServerException{
...
//		params.set("rows",rows);
	    ...
	    if (CollectionUtils.isNotEmpty(list)) {
	    	...
	    	for (int i = 0; i < list.size(); i++) {
	    		...
	    		retList.add((Long)document.get("liveid"));
	    	}
	    	return retList;
		}else {
	...
	public static void main(String []str){
		...
		try {
			...
			//s.search("\\?");
			SolrWoPaiLiveTagService server = new SolrWoPaiLiveTagService();
